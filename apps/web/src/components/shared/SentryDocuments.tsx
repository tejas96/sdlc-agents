...
import { SentryServicesModal } from '@/components/shared/SentryServicesModal';
...
interface SentryDocumentsProps {
  type: IncidentBasedType;
}

export function SentryDocuments({ type }: SentryDocumentsProps) {
  const { resetSentryConnection, sentryConnection } = useOAuth();
  const { accessToken } = useUser();
  const { incidentsentry, setIncidentsentry, resetIncidentsentry } =
    useProject();
  ...
  const content = {
    incident: {
      title: 'Sentry Incidents',
      emptyText: 'No Sentry incidents configured.',
      disconnectMessage:
        'Are you sure you want to disconnect Sentry? This will remove all incidents and their associated data. This action cannot be undone.',
    },
  };
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      toast.error('Failed to disconnect Sentry');
    } finally {
        ...
        {!incidentsentry.incident ? (
          ...
          <div className='space-y-3'>
            <div className='max-h-80 min-h-0 flex-1 overflow-auto bg-white'>
              {incidentsentry.incident && (
                <div
                  key={incidentsentry.incident.id}
                ...
                >
                  ...
                  <h4 className='mb-2 pr-8 text-sm font-medium text-gray-900'>
                    {incidentsentry.incident.title}
                  </h4>

                  {/* Description */}
                  <p className='mb-2 text-xs leading-relaxed text-gray-600'>
                    Incident details and information from Sentry error tracking
                    system.
                  </p>
                  ...
                  <div className='flex flex-wrap items-center gap-2'>
                    <Badge variant='outline' className='text-xs'>
                      Type: {incidentsentry.incident.type}
                    </Badge>
                    <Badge variant='outline' className='text-xs'>
                      Reported:{' '}
                      {incidentsentry.incident.last_seen
                        ? formatDistanceToNow(
                            new Date(incidentsentry.incident.last_seen),
                            {
                              addSuffix: true,
                            }
                          )
