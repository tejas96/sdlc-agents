...
This module provides the OAuth implementation for Discord integration.
Discord uses OAuth 2.0 for secure authentication and authorization,
allowing applications to access user data and perform actions on behalf of users.

OAuth Flow:
1. Authorization request to Discord OAuth endpoint
...
@register_oauth_provider(IntegrationProvider.DISCORD)
class DiscordOAuthProvider(OAuthProviderContract):
    """Discord OAuth provider implementation.

    This class handles OAuth 2.0 authentication flow for Discord integration.
    It manages authorization requests, token exchanges, and credential validation
    according to Discord's OAuth 2.0 specification.

    Attributes:
        client_id: The OAuth client ID for the Discord application
        client_secret: The OAuth client secret for the Discord application
        redirect_uri: The registered redirect URI for OAuth flow
        auth_url: Discord's OAuth authorization endpoint
        token_url: Discord's OAuth token endpoint
        scope: OAuth scopes for permissions
    """

    def __init__(self, credentials: dict[str, Any]) -> None:
        """Initialize the Discord OAuth provider with credentials.
        ...
        self.scope = credentials.get("scope", "identify guilds")
        self.auth_url = "https://discord.com/api/oauth2/authorize"
        self.token_url = "https://discord.com/api/oauth2/token"

    async def validate_credentials(self, credentials: dict[str, Any]) -> dict[str, Any]:
        """Validate OAuth credentials for Discord.

        Validates the provided OAuth credentials against Discord's requirements
        ...
        # Validate client_id format (Discord snowflake ID)
    ...
    async def generate_access_token(self, credentials: dict[str, Any]) -> TokenResult:
        ...
        Exchanges the authorization code received from Discord's OAuth flow
        ...
        # Send POST request to Discord token endpoint
    ...
    # Example: refresh_token(), revoke_token(), get_authorization_url(), get_user_info(), etc.
