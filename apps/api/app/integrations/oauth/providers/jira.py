...
This module provides the OAuth implementation for Atlassian Jira integration.
Jira supports OAuth 2.0 (3LO) for secure authentication and authorization,
allowing applications to access projects, issues, and user data.

OAuth Flow:
1. Authorization request to Atlassian OAuth endpoint
...
@register_oauth_provider(IntegrationProvider.JIRA)
class JiraOAuthProvider(OAuthProviderContract):
    """Jira OAuth provider implementation.

    This class handles OAuth 2.0 authentication flow for Jira integration.
    It manages authorization requests, token exchanges, and credential validation
    according to Atlassian's OAuth 2.0 (3LO) specification.

    Attributes:
        client_id: The OAuth client ID for the Atlassian application
        client_secret: The OAuth client secret for the Atlassian application
        redirect_uri: The registered redirect URI for OAuth flow
        auth_url: Atlassian's OAuth authorization endpoint
        token_url: Atlassian's OAuth token endpoint
        scope: OAuth scopes for permissions
    """

    def __init__(self, credentials: dict[str, Any]) -> None:
        """Initialize the Jira OAuth provider with credentials.
        ...
        self.scope = credentials.get("scope", "read:jira-work write:jira-work")
        self.auth_url = "https://auth.atlassian.com/authorize"
        self.token_url = "https://auth.atlassian.com/oauth/token"

    async def validate_credentials(self, credentials: dict[str, Any]) -> dict[str, Any]:
        """Validate OAuth credentials for Jira.

        Validates the provided OAuth credentials against Atlassian's requirements
    ...
    async def generate_access_token(self, credentials: dict[str, Any]) -> TokenResult:
        ...
        Exchanges the authorization code received from Atlassian's OAuth flow
        ...
        # Send POST request to Atlassian token endpoint
    ...
    # Example: refresh_token(), revoke_token(), get_authorization_url(), get_accessible_resources(), etc.
