...
This module provides the OAuth implementation for ClickUp integration.
ClickUp uses OAuth 2.0 for secure authentication and authorization,
allowing applications to access user data with proper permissions.

OAuth Flow:
1. Authorization request to ClickUp OAuth endpoint
...
@register_oauth_provider(IntegrationProvider.CLICKUP)
class ClickUpOAuthProvider(OAuthProviderContract):
    """ClickUp OAuth provider implementation.

    This class handles OAuth 2.0 authentication flow for ClickUp integration.
    It manages authorization requests, token exchanges, and credential validation
    according to ClickUp's OAuth 2.0 specification.

    Attributes:
        client_id: The OAuth client ID for the ClickUp application
        client_secret: The OAuth client secret for the ClickUp application
        redirect_uri: The registered redirect URI for OAuth flow
        auth_url: ClickUp's OAuth authorization endpoint
        token_url: ClickUp's OAuth token endpoint
    """

    def __init__(self, credentials: dict[str, Any]) -> None:
        """Initialize the ClickUp OAuth provider with credentials.
    ...
    async def validate_credentials(self, credentials: dict[str, Any]) -> dict[str, Any]:
        """Validate OAuth credentials for ClickUp.

        Validates the provided OAuth credentials against ClickUp's requirements
    ...
    async def generate_access_token(self, credentials: dict[str, Any]) -> TokenResult:
        ...
        Exchanges the authorization code received from ClickUp's OAuth flow
        ...
        # Send POST request to ClickUp token endpoint
