"""Datadog OAuth provider.

This module provides the OAuth implementation for Datadog integration.
Datadog uses OAuth 2.0 for secure authentication and authorization,
allowing applications to access monitoring data and manage resources.

OAuth Flow:
1. Authorization request to Datadog OAuth endpoint
...
@register_oauth_provider(IntegrationProvider.DATADOG)
class DatadogOAuthProvider(OAuthProviderContract):
    """Datadog OAuth provider implementation.

    This class handles OAuth 2.0 authentication flow for Datadog integration.
    It manages authorization requests, token exchanges, and credential validation
    according to Datadog's OAuth 2.0 specification.

    Attributes:
        client_id: The OAuth client ID for the Datadog application
        client_secret: The OAuth client secret for the Datadog application
        redirect_uri: The registered redirect URI for OAuth flow
        site: Datadog site (e.g., datadoghq.com, datadoghq.eu)
        auth_url: Datadog's OAuth authorization endpoint
        token_url: Datadog's OAuth token endpoint
    """

    def __init__(self, credentials: dict[str, Any]) -> None:
        """Initialize the Datadog OAuth provider with credentials.
        ...
        self.auth_url = f"https://app.{self.site}/oauth2/v1/authorize"
        self.token_url = f"https://api.{self.site}/oauth2/v1/token"

    async def validate_credentials(self, credentials: dict[str, Any]) -> dict[str, Any]:
        """Validate OAuth credentials for Datadog.

        Validates the provided OAuth credentials against Datadog's requirements
        ...
        # Validate Datadog site configuration
    ...
    async def generate_access_token(self, credentials: dict[str, Any]) -> TokenResult:
        ...
        Exchanges the authorization code received from Datadog's OAuth flow
        ...
        # Send POST request to Datadog token endpoint
